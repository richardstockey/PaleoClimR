...
cGENIE.trouble.plots.simple <- function(experiment, directory="default", file.name = "cGENIE.trouble.plots", format = "pdf", save = FALSE){
  ...
  sum <- ggarrange(bathymetry,
                   streamfunction,
                   ncol = 2)

  # Save the plot to a file if save is TRUE
  if(save == TRUE){
    ggsave(file = paste0(directory, "/", file.name, ".", format), sum, height = 10, width = 29.7, units = "cm")
  }
