...
cGENIE.transect <- function(experiment, var,
        ...
        projection = 'ESRI:54012'){
          ...
          RNetCDF::open.nc  # For handling NetCDF files
          dplyr::between    # For data manipulation
          reshape2::melt    # For melting arrays into data frames
          ggplot2::ggplot   # For creating plots
          pals::parula      # For color palettes
          ...
          lat <- RNetCDF::var.get.nc(nc, "lat")  # Latitude values
          lat.edges <- RNetCDF::var.get.nc(nc, "lat_edges")  # Latitude edge values
          lon <- RNetCDF::var.get.nc(nc, "lon")  # Longitude values
          lon.edges <- RNetCDF::var.get.nc(nc, "lon_edges")  # Longitude edge values
          depth <- RNetCDF::var.get.nc(nc, "zt")  # Depth values
          depth.edges <- RNetCDF::var.get.nc(nc, "zt_edges")  # Depth edge values
          time <- RNetCDF::var.get.nc(nc, "time")  # Time values
          var.arr <- RNetCDF::var.get.nc(nc, var)  # Extract the specified variable
          ...
          if(dims == 3 && lat.or.lon == "lat"){
            df <- as.data.frame(cbind(
              ...
              as.data.frame(reshape2::melt(var.arr[slice,,, time.step]))$value
            ))
          ...
          if(lat.or.lon == "lat"){
            map <- ggplot2::ggplot(data = df, aes(x = lat.mid, y = depth.mid, xmin = lat.min, xmax = lat.max, ymin = depth.min, ymax = depth.max, fill = var * unit.factor)) +
              ggplot2::geom_rect(color = NA, linewidth = 10, linetype = 0) +
    ...
    ggplot2::xlab("Latitude (\u00B0)") +
    ggplot2::labs(fill = scale.label, title = paste0(lon[slice], "\u00B0 Longitude"))
    }

    # Create the plot for longitude transect
    if(lat.or.lon == "lon"){
    map <- ggplot2::ggplot(data = df, aes(x = lon.mid, y = depth.mid, xmin = lon.min, xmax = lon.max, ymin = depth.min, ymax = depth.max, fill = var * unit.factor)) +
      ggplot2::geom_rect(color = NA, linewidth = 10, linetype = 0) +
      ggplot2::scale_fill_stepsn(colours = palette_name,
          guide = ggplot2::guide_colorbar(title.position = "top",
                 barwidth = 12,
      ...
      ggplot2::labs(fill = scale.label, title = paste0(lat[slice], "\u00B0 Latitude"))
    }
