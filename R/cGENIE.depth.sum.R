...
cGENIE.depth.sum <- function(var, experiment, year = "default", model = "biogem") {
  ...
  library(dplyr)     # For data manipulation (group_by, summarise, na.omit)
  ...
  var.depth.sum <- var.3D.df %>%
    ...
    summarise(mean.var = mean(var, na.rm = TRUE),  # Mean of the variable at each depth level
              sd.var = sd(var, na.rm = TRUE)) %>%  # Standard deviation at each depth level
