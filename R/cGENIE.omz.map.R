...
cGENIE.omz.map <- function(experiment,
  ...
  arr.3d.omz <- filter(arr.3d, var <= thresh)

  arr.3d.omz$cell.thickness <- arr.3d.omz$depth.max - arr.3d.omz$depth.min

  lon.lat.omz <- arr.3d.omz %>%
  group_by(lon.min, lon.max, lat.max, lat.min) %>%
  summarise(var = sum(cell.thickness)) %>%
  as.data.frame()
  ...
  polygons.code <- Polygons(list(polygon.code), paste0("land", poly))
  ...
  LandSpP <- SpatialPolygons(land.poly.list)
  land.attr <- data.frame(land = land.cells$land, row.names = land.poly.names.list)
  LandSpDf <- SpatialPolygonsDataFrame(LandSpP, land.attr)
  LandSpDfSf <- st_as_sf(LandSpDf)
  st_crs(LandSpDfSf) = '+proj=longlat +ellps=sphere'

  if (continents.outlined == TRUE) {
  continent_polygons <- df2 %>% filter(is.na(var))
  ...
  map <- ggplot() +
    geom_sf(data = SLs1dfSf %>% st_transform(projection), color = "grey20", linewidth = line.thickness, fill = "white") +
    ...
    theme(legend.position = "bottom",
      ...
      panel.grid.major = element_line(color = fg.color),
      panel.grid.minor = element_line(color = fg.color),
      axis.text = element_text(color = fg.color),
      axis.title = element_text(color = fg.color),
      legend.text = element_text(color = fg.color),
      legend.title = element_text(color = fg.color)) +
    labs(fill = scale.label)

  } else {
  # Create the map using ggplot
  map <- ggplot() +
      geom_sf(data = SLs1dfSf %>% st_transform(projection), color = "grey5", linewidth = 0.9, fill = "white") +
    geom_sf(data = SpDfSf %>% st_transform(projection), aes(fill = var * unit.factor), color = NA) +
    geom_sf(data = LandSpDfSf %>% st_transform(projection), fill = "grey90", color = NA) +
